  ...
  return <Modal visible transparent animationType="slide" onRequestClose={onFechar} statusBarTranslucent>
    <View style={s.backdrop}>
      ...
      <View style={s.sheet} accessibilityViewIsModal>
        <View style={s.handle} />
        <View style={s.header}><View><Text style={s.kicker}>Carteira de vacinação</Text><Text style={s.title} numberOfLines={1}>{pet.nome}</Text></View><Pressable style={s.close} onPress={onFechar} hitSlop={10} accessibilityLabel="Fechar"><Ionicons name="close" size={22} color={C.text} /></Pressable></View>
        <ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
          <View style={s.identity}><View style={s.identityIcon}><Ionicons name="shield-checkmark" size={27} color={C.green700} /></View><View style={s.identityInfo}><Text style={s.identityName}>{pet.nome}</Text><Text style={s.identityMeta}>{especie?.label ?? 'Espécie não informada'}{pet.raca ? ` · ${pet.raca}` : ''}</Text></View></View>
          <View style={s.stats}><Resumo label="Registros" valor={vacinas.length} /><Resumo label="Realizadas" valor={realizadas} /><Resumo label="Agendadas" valor={proximas} /></View>
          <View style={s.sectionHead}><View><Text style={s.sectionTitle}>Histórico de vacinas</Text><Text style={s.sectionSub}>Registros enviados pela clínica</Text></View><Ionicons name="medical-outline" size={20} color={C.green700} /></View>
          {vacinas.length === 0 ? <View style={s.empty}><Ionicons name="document-text-outline" size={25} color={C.green600} /><Text style={s.emptyTitle}>Nenhuma vacina registrada</Text><Text style={s.emptyText}>Quando a clínica registrar uma vacinação, ela aparecerá aqui.</Text></View> : <View style={s.list}>{vacinas.map((evento, index) => <Registro key={evento.id} evento={evento} ultimo={index === vacinas.length - 1} />)}</View>}
          <Text style={s.disclaimer}>Esta carteira exibe somente registros disponíveis na sua conta. Para incluir ou corrigir uma vacina, fale com a clínica responsável.</Text>
        </ScrollView>
      </View>
    </View>
  </Modal>;
...
function Resumo({ label, valor }: { label: string; valor: number }) { return <View style={s.summary}><Text style={s.summaryValue}>{valor}</Text><Text style={s.summaryLabel}>{label}</Text></View>; }
function Registro({ evento, ultimo }: { evento: Evento; ultimo: boolean }) {
  const visual = statusDoEvento(evento);
  return <View style={[s.record, ultimo && s.recordLast]}><View style={s.recordIcon}><Ionicons name="medical" size={16} color={C.green700} /></View><View style={s.recordInfo}><Text style={s.recordName}>{evento.nomeTipoEvento}</Text><Text style={s.recordMeta}>{formatarDataEvento(evento.data)}{evento.nomeVeterinario && evento.nomeVeterinario !== '—' ? ` · ${evento.nomeVeterinario}` : ''}</Text>{evento.observacao ? <Text style={s.recordNote}>{evento.observacao}</Text> : null}</View><View style={[s.status, { backgroundColor: visual.fundo }]}><Text style={[s.statusText, { color: visual.cor }]}>{visual.texto}</Text></View></View>;
}
