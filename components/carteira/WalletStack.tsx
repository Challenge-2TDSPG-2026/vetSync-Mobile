...
export function WalletStack({ pets, petAtivoId, onSelecionar, onTrocarPetAtivo }: Props) {
  ...
  return <View style={s.container} accessibilityLabel="Carteiras de vacinação dos pets">
    {pets.map((pet, index) => {
      ...
      return <Pressable key={pet.id} onPress={() => { onTrocarPetAtivo?.(pet.id); onSelecionar(pet); }} accessibilityRole="button" accessibilityLabel={`Abrir carteira de vacinação de ${pet.nome}`} style={({ pressed }) => [s.pass, modoIdoso && sIdoso.pass, ativo && s.passAtivo, pressed && s.pressed]}>
        <View style={[s.iconBox, modoIdoso && sIdoso.iconBox]}><AppIcon name={especie?.icon ?? 'paw'} set={especie?.iconSet ?? 'MaterialCommunityIcons'} size={modoIdoso ? 26 : 21} color={C.green700} /></View>
        <View style={s.info}>
          <View style={s.nameLine}><Text style={[s.name, modoIdoso && sIdoso.name]} numberOfLines={1}>{pet.nome}</Text>{ativo && <Text style={s.activeBadge}>Ativa</Text>}</View>
          <Text style={[s.meta, modoIdoso && sIdoso.meta]} numberOfLines={1}>{descricaoPet(pet)}</Text>
          {!modoIdoso && <Text style={s.document}>Carteira de vacinação digital</Text>}
        </View>
        <View style={s.action}><Text style={[s.actionText, modoIdoso && sIdoso.actionText]}>Ver</Text><Ionicons name="chevron-forward" size={modoIdoso ? 21 : 17} color={C.green700} /></View>
        {!modoIdoso && <Text style={s.index}>{String(index + 1).padStart(2, '0')}</Text>}
      </Pressable>;
    })}
  </View>;
}
...
/** Overrides do modo idoso: cartões maiores, sem os detalhes decorativos menores (índice/legenda). */
const sIdoso = StyleSheet.create({
  pass: { minHeight: 116, padding: 18 },
  iconBox: { width: 56, height: 56, borderRadius: 28 },
  name: { fontSize: 19 },
