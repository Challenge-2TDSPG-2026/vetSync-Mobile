...
function TabItem({ label, icon, active, onPress, onLongPress, idoso }: { label: string; icon: React.ReactNode; active: boolean; onPress: () => void; onLongPress: () => void; idoso?: boolean }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityState={{ selected: active }} accessibilityLabel={label} onPress={onPress} onLongPress={onLongPress} style={s.item}>
    <Animated.View style={[s.iconWrap, idoso && sIdoso.iconWrap, active && s.iconWrapActive, { transform: [{ translateY }, { scale }] }]}>{icon}</Animated.View>
    <Text numberOfLines={1} style={[s.label, idoso && sIdoso.label, active && s.labelActive]}>{label}</Text>
  </Pressable>;
}

function AssistantItem({ idoso }: { idoso?: boolean }) {
  const router = useRouter();
  return <Pressable accessibilityRole="button" accessibilityLabel="Abrir a SIA" onPress={() => router.push('/assistente')} style={s.item}>
    <View style={[s.iconWrap, idoso && sIdoso.iconWrap]}><Ionicons name="sparkles-outline" size={idoso ? 26 : 21} color={C.inactive} /></View>
    <Text style={[s.label, idoso && sIdoso.label]}>IA</Text>
  </Pressable>;
}
...
export function ClyvoTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  const altura = (modoIdoso ? 82 : 70) + Math.max(insets.bottom, 6);
  return <View style={[s.shell, { paddingBottom: Math.max(insets.bottom, 6), height: altura }]}>
    <View style={s.bar}>
      {routesVisiveis.map((route, index) => {
        ...
        const icon = options.tabBarIcon?.({ focused: active, color, size: modoIdoso ? 26 : 21 });
        ...
        return <React.Fragment key={route.key}>
          {index === indiceDaIa && <AssistantItem idoso={modoIdoso} />}
          <TabItem label={label} icon={icon} active={active} onPress={handlePress} onLongPress={handleLongPress} idoso={modoIdoso} />
        </React.Fragment>;
      })}
      {indiceDaIa === routesVisiveis.length && <AssistantItem idoso={modoIdoso} />}
    </View>
  </View>;
}
...
/** Overrides do modo idoso: ícones e rótulos maiores, área de toque maior. */
const sIdoso = StyleSheet.create({
  iconWrap: { width: 48, height: 48, borderRadius: 24 },
